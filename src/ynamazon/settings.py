...
class Settings(BaseSettings):
    ...
    ynab_approved_statuses: list[Literal["approved", "unapproved"]] = [
        "approved",
        "unapproved",
    ]  # Which approval statuses to match
    auto_accept_date_mismatch: bool = (
        False  # Auto-accept date mismatches without prompting (useful for daemon mode)
    )
    ...
    apprise_urls: SecretStr = SecretStr(
        ""
    )  # Comma-separated Apprise notification URLs (empty = no notifications)
